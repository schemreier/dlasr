...
if 'VIRTUAL_ENV' in os.environ:
    ...
    ROOT = os.environ['VIRTUAL_ENV'] + "/dlasr.clam/"
    PORT = 8802
    ...
    if host == 'applejack': #configuration for server in Nijmegen
        ...
        if not 'CLAMTEST' in os.environ:
            ...
            ROOT = "/scratch2/www/webservices-lst/test/writable/dlasr/"
            PORT = 81
...
PROFILES = [
    Profile(
        ...
        #------------------------------------------------------------------------------------------------------------------------
        OutputTemplate('Alignment',PlainTextFormat,'Recognition output in csv format',
            SetMetaField('encoding','ascii'), #note that encoding is required if you work with PlainTextFormat
            extension='.csv', #set an extension or set a filename:
...
#
# COMMAND = WEBSERVICEDIR + "/dlasr_wrapper.sh $DATAFILE $STATUSFILE $OUTPUTDIRECTORY"
...
COMMAND = WEBSERVICEDIR + "/dlasr_wrapper.sh $STATUSFILE $INPUTDIRECTORY $OUTPUTDIRECTORY " + SCRATCHDIRECTORY +" "+RESOURCESDIRECTORY +" "+ WEBSERVICEDIR
